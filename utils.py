...
def _sync_task(task_id, fact_ids, activity_id, duration, date):
    ...
    if not _facts_synced(task_id, fact_ids):
        print 'going to log task %s' % task_id
    ...
    for k,v in sorted(tasks.items()):
        for e in v:
            if _task_exists(e['task_id']):
                ...
                print '%s does not exist' % e['task_id']

def _check_tasks(tasks):
    """Checking if given facts are already synchronized."""
    
    for k,v in sorted(tasks.items()):
        print 'tasks logged on: %s' % k.isoformat()
        for task in v:
            if not _fact_synced(task['task_id'], task['fact_id']):
                print 'task %s, fact id %s was not synchronized' % (task['task_id'], task['fact_id'])
            else:
                print 'task %s, fact id %s was synchronized' % (task['task_id'], task['fact_id'])
